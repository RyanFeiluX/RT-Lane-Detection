...
def get_args():
    ...
    parser.add_argument('--test-list', dest='test_list', default = None, type = str)
    parser.add_argument('--test-cfg', dest='test_cfg', default='CULane', choices=['CULane', 'Tusimple'])
    parser.add_argument('--num_lanes', default = None, type = int)
    parser.add_argument('--frame-interval', dest='frame_interval', default=1, type=int)
    parser.add_argument('--video-in', dest='video_in', default=None, type=str)
    parser.add_argument('--video-out', dest='video_out', default=None, type=str)
...
def merge_config():
    ...
    if args.dataset != 'Custom':
        ...
        if args.test_cfg == 'CULane':
            cfgref = 'configs/culane.py'
        elif args.test_cfg == 'Tusimple':
            cfgref = 'configs/tusimple.py'
        else:
            raise RuntimeError('Invalid argument --test_cfg: %s' % args.test_cfg)
        cfg = Config.fromfile(config)
        cref = Config.fromfile(cfgref)
        for key in cref.keys():
            dist_print('copy', key, cfgref)
            setattr(cfg, key, getattr(cref, key))
